...
macro_rules! define_slice_matcher {
	($ty:ty, $matched:ty, |$matcher:ident, $slice:ident| ($logic:expr, $expected:expr)) => {
		impl Matcher<$matched> for $ty {
			type Capture<'src> = <$matched as MatchAble>::Slice<'src>;
			fn do_match<'src, M: Mode>(
				&self, matched: &'src $matched, off: &mut usize,
			) -> MatchResult<Self::Capture<'src>, M> {
				let $matcher = self;
				match_slice::<M, _, _>(matched, off, self.len(), 
					|$slice| $logic.then_some($slice), 
					|| <_ as Matcher<$matched>>::expected(&self)
				)
			}
			fn expected(&self) -> Expected {
				let $matcher = self;
...
macro_rules! define_token_matcher {
	($ty:ty, $matched:ty, |$matcher:ident, $token:ident| ($logic:expr, $expected:expr)) => {
		impl Matcher<$matched> for $ty {
			type Capture<'src> = <$matched as MatchAble>::Slice<'src>;
			fn do_match<'src, M: Mode>(
				&self, matched: &'src $matched, off: &mut usize,
				...
				let $matcher = self;
				let expected = || <_ as Matcher<$matched>>::expected(&self);
				match_token::<M, _>(matched, off, |$token| $logic, expected)
			}
			fn expected(&self) -> Expected {
				let $matcher = self;
				$expected
...
macro_rules! define_ref_matcher {
	($(#for ($($args:tt)+))? $ty:ty, for $matched:ty) => {
		impl $(<$($args)+>)? Matcher<$matched> for $ty {
			type Capture<'src> = <$matched as MatchAble>::Slice<'src>;
			fn do_match<'src, M: Mode>(
				&self, matched: &'src $matched, off: &mut usize,
			) -> MatchResult<Self::Capture<'src>, M> {
				AsRef::<$matched>::as_ref(self).do_match::<M>(matched, off)
			}
			fn expected(&self) -> Expected {
				<_ as Matcher<$matched>>::expected(AsRef::<$matched>::as_ref(self))
			}
		}
	};
}
pub(crate) use {define_ref_matcher, define_slice_matcher, define_token_matcher};
