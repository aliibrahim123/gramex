...
use crate::{
	cursor::{Error, err},
	parse::{Atom, CapType, Capture, Expr, Rep},
};
	...
	match expr {
		...
		Expr::Unit { not, rep, atom: Atom::Group(expr), .. } => {
			if *not || *rep != Rep::ONCE && *rep != Rep::OPTIONAL {
				forbid_captures(expr, ctx)
			} else {
				resolve_captures(expr, (*rep == Rep::OPTIONAL) | is_optional, parent, ctx);
			};
		}
		Expr::Capture(cap) => {
			resolve_capture(&mut *cap, is_optional, parent, ctx);
		}
		_ => {}
	}
}
fn forbid_captures(expr: &Expr, ctx: &mut Context) {
	match expr {
		...
		Expr::Imply { cond, expr } => {
			forbid_captures(cond, ctx);
			forbid_captures(expr, ctx);
		}
		Expr::Unit { atom: Atom::Group(expr), .. } => forbid_captures(expr, ctx),
		Expr::Unit { atom: Atom::Call { args, .. }, .. } => {
			for arg in args {
				forbid_captures(&arg.expr, ctx);
			}
		}
		Expr::Capture(cap) => {
			err!(ctx, "capture not allowed here", cap.ident.span());
		}
		_ => {}
	}
}
...
fn resolve_capture_type(
	...
	let mut resolve_gen_item = |item_ident: Option<_>, create: fn(_) -> _| -> Result<_, ()> {
		...
		let item_ident = item_ident.unwrap_or_else(|| camel_case(&cap.ident));
...
fn resolve_enum_capture(
	...
	for expr in exprs {
		...
		has_none |= child.is_none();
		if let Some(CapChild { name, resolved_type, container }) = &child {
			if let Some(mut def) = variants_def.as_mut() {
				chunk!(def, #{camel_case(&name)}(#do {container.wrap_type(def, resolved_type)}),)
			}
		}
		variants.push(child.map(|c| c.name));
	}
...
fn resolve_capture(
	...
	let kind = if has_capture(&cap.expr) {
		...
		match create {
			...
			_ if is_atomic_unit(&cap.expr) => CapKind::Atomic { need_from },
