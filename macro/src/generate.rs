...
pub fn gen_term(mut stream: &mut TokenStream, term: &Term, matched_type: &TokenStream) {
	...
	chunk!(stream,
		# #[doc = "[`Matcher`](gramex::Matcher) generated by [`grammar`](gramex::grammar)"]
		#if args.is_empty() #{
...
fn gen_variant_do_match(
	mut stream: &mut TokenStream, matcher: &EnumMatcher, var: &Variant,
) {
	let EnumMatcher { name: enum_, field, .. } = matcher;
	let Variant { name, fields } = var;
	let fields = fields.as_ref().map(|fields| {
		token!((#for (i, _) in fields.iter().enumerate() #{
			#if i > 0 #{ , } #{ident!("field_{i}")}
		}))
	});
	chunk!(stream,
		...
		if let #enum_::#name
			#if let Some(fields) = &fields #{ #fields } #else #{ { .. } }
		= &token #field {
			*off += 1;
			Ok(M::wrap_success(
				#if fields.is_some() #{ #fields } #else #{ token }
			))
		} else { M::err(||
...
pub fn gen_enum_matcher(mut stream: &mut TokenStream, matcher: &EnumMatcher) {
	let EnumMatcher { matched_type, vars, expected, name: enum_, .. } = matcher;
	for var in vars {
		let camel_name = camel_case(&var.name);
		let matcher_ident = ident!("{}__Matcher", span = var.name.span(), var.name);
		chunk!(stream,
			# #[doc = concat!(
				"kind [`Matcher`](gramex::Matcher) for [`",
				#{enum_.to_string()}, "::", #{var.name.to_string()},
				"`]\n\n",
				"generated by [`derive_enum_matcher`](gramex::derive::derive_enum_matcher)"
			)]
			# #[allow(nonstandard_style)]
			...
			impl<'slice> ::gramex::Matcher<#matched_type> for #matcher_ident {
				type Capture<'src> = #match &var.fields {
					Some(fields) => #{
				...
				fn do_match<'src, M: ::gramex::Mode>(
					...
					#do { gen_variant_do_match(stream, matcher, var, ) }
				}

				fn expected(&self) -> ::gramex::result::Expected {
					#if let Some(expected) = expected #{
						#expected(#{var.name.to_string()})
					} #else #{
						::gramex::result::Expected::None
					}
				}
			}
		);
